...
export const CreateMatch = (props) => {
  ...
  const handleSubmit = async (e) => {
    ...
    e.preventDefault();
    if (newMatchDate.trim() === "") return;

    if (newMatchDivisionID.trim() === "") return;

    if (newMatchRoundID.trim() === "") return;

    if (newMatchStartTime.trim() === "") return;

    if (newMatchTeam1.trim() === "") return;

    if (newMatchTeam2.trim() === "") return;

    if (
      newMatchRoundID === "QUARTERFINALS 1" ||
      newMatchRoundID === "QUARTERFINALS 4"
    ) {
            ...
            <div className="d-flex justify-content-around">
              <form onSubmit={handleSubmit} className="justify-content-around">
                <Row className="justify-content-around">
                  ...
                  <DivisionList
                    setValue={newMatchDivisionID}
                    setFunction={setNewMatchDivisionID}
                  />
                  <RoundList
                    setValue={newMatchRoundID}
                    setFunction={setNewMatchRoundID}
                  />
                  <TeamList
                    setValue={newMatchTeam1}
                    setFunction={setNewMatchTeam1}
                    event={event}
                  />
                  <TeamList
                    setValue={newMatchTeam2}
