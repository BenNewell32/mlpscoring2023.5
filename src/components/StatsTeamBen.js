...
export const StatsTeamBen = (props) => {
    ...
    useEffect(() => {;
        (async () => {
            const listMatches = query(collection(db, "mlpStandingsTeam"), where("division", "==", division), where("event", "==", location));

            const listTeams = query(collection(db, "mlpteams"), where("divisionID", "==", division), where("eventID", "==", location));
            ...
            for (let i = 0; i < mlpTeamsList.length; i++) { // console.log(mlpTeamsList[i].teamName)
                for (let r = 0; r < teamsList[0].teamRank.length; r++) { // console.log(teamsList[0].teamRank[r].team)
            ...
            for (let i = 0; i < teamsList[0].teamRank.length; i++) { // console.log(teamsList[0].teamRank[i].team);
